...
f_shortcuts_folder = %A_MyDocuments%\Favorites
;f_shortcuts_folder = D:\Documents and Settings\Recent
...
Menu, Tray, Add, Add Folder to Favorites, f_AddFolder
; Add a command to capture the current application.
Menu, Tray, Add, Add File to Favorites, f_AddFile
; Add a command to allow the user to edit the shortcuts.
Menu, Tray, Add, Edit Favorites Folder, f_EditFavorites
; Add a command to allow the user to edit the shortcuts.
Menu, Tray, Add, About..., f_AboutScript
; Add a separator line.
Menu, Tray, Add  
; Add a command to allow the user to hide tray icon
Menu, Tray, Add, Hide Tray Icon, f_HideTrayIcon
...
{	
    ...
    ; Add a command to open "Program Files" special folder.
    ;Menu, %a%, Add, Program Files, f_Program_Files
...
IfNotExist,%name_no_ext%.ini
{
  ini=`;%name_no_ext%.ini
  ini=%ini%`n;
  ini=%ini%`n;常用组合示例
  ini=%ini%`n;MButton - 按鼠标中键弹出菜单
  ini=%ini%`n;^MButton - 按Ctrl+鼠标中键弹出菜单
  ini=%ini%`n;^F12 - 按Ctrl+F12弹出菜单
  ini=%ini%`n;!F12 - 按Alt+F12弹出菜单  
  ini=%ini%`n;+F12 - 按Shift+F12弹出菜单  
  ini=%ini%`n;#Space - 按Win+Space弹出菜单    
  ini=%ini%`n;
  ini=%ini%`n;最前面的~表示同时触发原来对应的热键
  ini=%ini%`n;
  ini=%ini%`n[Settings]
  ini=%ini%`n;Ctrl+鼠标中键  
  ini=%ini%`nhotkey1=~^MButton
  ini=%ini%`n;Ctrl+AppsKey 
  ini=%ini%`nhotkey2=^AppsKey
  FileAppend,%ini%,%name_no_ext%.ini
  ini=
	  ...
	  If ErrorLevel 
		  MsgBox, 16, Install %Gui1WinTitle%, 
			(LTrim 
			  ...
			  你可以自己在“开始”菜单的“启动”文件夹中创建此文件的快捷方式: 
			  %A_ScriptDir%\%A_ScriptName% 
			) 
	} 

  ABOUT:
  about=欢迎使用“Quick Shotcuts”!
  about=%about%`n
  about=%about%`n要修改快捷键，请编辑 %name_no_ext%.ini 文件.
  about=%about%`n
  about=%about%`n程序第一次运行时会在“开始”菜单的“启动”文件夹中创建自启动快捷方式，如果没有请自己根据需要创建.
  about=%about%`n
  about=%about%`nPeter Liu
  MsgBox,0,Shortcuts,%about%
  about=
}
IniRead,f_Hotkey1,%name_no_ext%.ini,Settings,hotkey1
IniRead,f_Hotkey2,%name_no_ext%.ini,Settings,hotkey2
...
If not A_IsCompiled
{
	IfExist %A_ScriptFullPath%.ico
		Menu, Tray, Icon, %A_ScriptFullPath%.ico
...
RELOADSCRIPT:
    ...
    IfInString, attribs, A
    {
        ...
        ;ToolTip,Updated script,%A_CaretX%,%A_CaretY%
        Sleep, 500
        SplashTextOff
        Reload
    }
...
f_CreateMenu:
  ...
  ; Add a command to allow the user to edit the shortcuts.
  Menu, %f_shortcuts_folder%, Add, About..., f_AboutScript
...
BuildMenu( testPath, menu )
{
    ...
    If f_NotFirstTime = True
    {
        ; In case this is a new folder, we need to add something first. 
        ; So that we won't delete a null menu in the following line.
        Menu, %MenuName%, add
...
f_Open_Target( target )
{
    ...
    ; PETER!
    ; These exceptions take care of some of the special folders - more to do.
    If target =  ; Desktop
        target = %A_Desktop%
    Else If target = My Documents
        target = %A_MyDocuments%
    Else IfNotExist, %target%
        OutputDebug Target Not Exist: %target%
...
f_My_Documents:
  f_Open_Target( A_MyDocuments )
  ;f_Open_Target( "::{450D8FBA-AD25-11D0-98A8-0800361B1103}" )
Return

f_Program_Files:
  f_Open_Target( A_ProgramFiles )
Return
...
f_FindPath( type )
{
    ...
    ; PETER!
    ; Third, Get the current path of a Window Explorer
    WinGetClass wClass, A 
    If (wClass == "ExploreWClass" || wClass == "CabinetWClass") 
    {
      ControlGetText currentPath, Edit1, ahk_class %wClass%
      good := f_TestWholeString( currentPath, type )
      If good
      {
          f_AddToFavorites( currentPath, type )
          Return
      }
      
      ; If you open a new window where the address bar is initially hidden you can't access it until you make it visible, so try another way.
      hWnd := WinExist("A") 
      currentPath := GetWindowsExplorerPath(hWnd)
      good := f_TestWholeString( currentPath, type )
      If good
      {
          f_AddToFavorites( currentPath, type )
    ...
    ; Fourth, test clipboard contents for valid path.
    ; PETER!
    ;clip = %Clipboard%
...
f_AboutScript:
  about=%about%`n========================================================
  about=%about%`nQuick Shortcuts: shortcuts popup menu that you can change on the fly. 
  about=%about%`n============================================================
  about=%about%`n
  about=%about%`nFeatures: 
  about=%about%`n
  about=%about%`n- 可以随时把你当前的文件夹或者可执行程序的快捷方式加入收藏
  about=%about%`n
  about=%about%`n- 按[鼠标中键]弹出菜单，方便你访问常用的文件和文件夹
  about=%about%`n
  about=%about%`n  也可以使用快捷键 RWin(右边的Windows徽标键) 弹出菜单
  about=%about%`n
  about=%about%`n- 可以对收藏夹进行整理，就像整理普通的文件夹一样
  about=%about%`n  
  about=%about%`n    
  about=%about%`n$$$$$$$$$ ====== CBTS开发部 效率提升推进小组 ====== $$$$$$$$$
  about=%about%`n$$$ADE$$$         ENJOY YOUR WORK AND LIFE!         $$$ADE$$$
  about=%about%`n$$$$$$$$$ ====== ** liude67596@huawei.com ** ====== $$$$$$$$$
  MsgBox,0,QuickFolders,%about%
  about=
Return
...
; http://www.autohotkey.com/forum/topic10273.html
